...
func TestConcurrentWithdrawalsNeverOverdraw(t *testing.T) {
	...
	cust, err := custSvc.Register(ctx, "Concurrency Test", "concurrency@example.com", "s3cret-password")
	if err != nil {
	...
	const attempts = 20
	const each = 1000 // 10.00 each; 100.00 / 10.00 = 10 should succeed
	var succeeded int64
	...
	if balance != 0 {
		t.Fatalf("final balance = %d, want 0 (never negative, never short)", balance)
	}
}
